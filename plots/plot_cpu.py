#!/usr/bin/env python3
"""Plot CPU usage over time from a cpu_mem.csv produced by cpu_memory_logger.py."""
...
def main():
    ap = argparse.ArgumentParser(description="Plot CPU usage from a run CSV.")
    ...
    plt.figure()
    plt.plot(df["elapsed_time_sec"], df["cpu_percent_total"])
    plt.xlabel("Elapsed time (s)")
    plt.ylabel("CPU usage (%)")
    plt.title(f"CPU usage — {run_id}")
    plt.grid(True, alpha=0.3)

    out = args.out or (os.path.splitext(path)[0] + "_cpu.png")
